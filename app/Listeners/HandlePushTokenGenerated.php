...
class HandlePushTokenGenerated
{
    public function handle(TokenGenerated $event): void
    {
        ...
        if (!$serverUrl || !$apiToken) {
            Log::warning("[PushNotification] Cannot sync token — missing config.");
            return;
        }

        try {
            // Send the raw FCM token directly to the remote server
            $response = Http::withToken($apiToken)
                ->post(rtrim($serverUrl, '/') . '/api/mobile/push-token', [
                    'push_token' => $token,
                ]);

            Log::info("[PushNotification] FCM Token sync response: {$response->status()}");
        } catch (\Throwable $e) {
            Log::error("[PushNotification] FCM Token sync failed: {$e->getMessage()}");
        }
    }
}
