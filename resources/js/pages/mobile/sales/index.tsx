...
export default function MobileSales() {
    ...
    useEffect(() => {
        if (serverUrl) fetchSales();
    }, [serverUrl]);

    const fetchSales = async () => {
        setLoading(true);
        try {
            const res = await remoteApi.get(`${serverUrl}/api/mobile/sales`);
    ...
    const getStatusIcon = (status: string) => {
        switch (status) {
            ...
            default: return <ShoppingCart className="w-5 h-5 text-blue-600" />;
        }
    };
    ...
    const getStatusBg = (status: string) => {
        switch (status) {
            ...
            case 'approved': return 'bg-green-100 border-green-200 dark:bg-green-900/30 dark:border-green-800';
            case 'cancelled': return 'bg-red-100 border-red-200 dark:bg-red-900/30 dark:border-red-800';
    ...
    return (
        <MobileLayout 
            title="Sales" 
            onSearch={(q) => console.log('Search sales', q)}
            fab={{
        ...
        >
            <div className="pb-4">
                <p className="text-xs font-semibold uppercase tracking-wider text-muted-foreground mb-3 px-2">Recent Transactions</p>
                                ...
                                <div key={item.id} className="flex items-center gap-4 p-3 hover:bg-muted/50 rounded-xl transition-colors active:bg-muted">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <div className="flex justify-between items-start">
                                            <p className="font-semibold text-[15px] truncate pr-2">Order #{item.id}</p>
                                            <p className="text-[13px] font-bold shrink-0">${item.total_amount ?? '0.00'}</p>
                                        </div>
                                        <div className="flex items-center justify-between mt-0.5">
                                            <p className="text-[13px] text-muted-foreground truncate capitalize">{item.status}</p>
