...
export default function PrintList({ sales, filters }: { sales: Sale[], filters: any }) {
    useEffect(() => {
        const attemptPrint = async () => {
            const nativeTriggered = await handleNativePrintFallback('printable-sales-list', `sales_report_${format(new Date(), 'yyyyMMdd')}`);
            if (!nativeTriggered) {
                window.print();
            }
        };
    ...
    const handleManualPrint = async () => {
        const nativeTriggerेड = await handleNativePrintFallback('printable-sales-list', `sales_report_${format(new Date(), 'yyyyMMdd')}`);
        if (!nativeTriggerेड) {
            window.print();
        }
    };
