...
export default function PrintList({ sales, filters }: { sales: Sale[], filters: any }) {
    useEffect(() => {
        const attemptPrint = async () => {
            const nativeTriggered = await handleNativePrintFallback('printable-sales-list', `sales_report_${new Date().toISOString().split('T')[0]}.png`);
            if (!nativeTriggered) {
                window.print();
            }
        };
    ...
    const handleManualPrint = async () => {
        const nativeTriggerेड = await handleNativePrintFallback('printable-sales-list', `sales_report_${new Date().toISOString().split('T')[0]}.png`);
        if (!nativeTriggerेड) {
            window.print();
        }
    };
    ...
    return (
        <div className="min-h-screen bg-white relative">
            ...
            <div id="printable-sales-list" className="p-8 max-w-5xl mx-auto print:p-0 print:max-w-none bg-white">
                {/* Header Section */}
